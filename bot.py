...
class CookieBot:
    ...
    def ensure_boosts_selected(self, items=None):
        """
        ติ๊กบูสต์ก่อนเริ่มเกม (default = ทั้ง 3 ใน config.BOOST_ITEMS):
          - ไอเทมไหนยังไม่เห็นเครื่องหมายถูก → กดเปิดใช้
          - เห็นถูกแล้ว → ข้าม (กันกดซ้ำจน toggle ปิด)
        items = รายการบูสต์ที่ต้องติ๊ก (โหมด exp ส่งมาเฉพาะ Star x2)
        เช็คซ้ำได้สูงสุด 3 รอบ (เผื่อรอแอนิเมชัน)
        """
        if items is None:
            items = config.BOOST_ITEMS
        if not items:
            self.log("[*] ไม่มีบูสต์ให้ติ๊กสำหรับโหมดนี้ → ข้าม")
            return
        self.log(f"[*] ตรวจสอบ Boost {len(items)} ไอเทม...")
        for _ in range(3):
            screen = self._cap()
            if screen is None:
            ...
            for item in items:
                ...
                all_on = False
            if all_on:
                self.log(f"[*] Boost ครบทั้ง {len(items)} อันแล้ว")
                return
    ...
    def state_run_box(self):
        ...
        if exp_mode:
            self.log("\n===== [STATE 2] RUN — เปิด Fast Start + ปล่อยวิ่งเอง (ไม่กระโดด) + คอยกดนินจา relay =====")
        else:
