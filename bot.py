...
โหมด "box" (วิ่งเก็บกล่อง — Fast Start):
  STATE 1 PREP   : นำทางไปหน้าเตรียมตัว → ซื้อ Fast Start (เฉพาะเมื่อเปิดใช้
                   TAP_FAST_START_ACTIVATE — ปิดคือไม่ซื้อ/ไม่กดเลย)
                   → (ถ้าเลือกไว้) Multi-Buy กล่องสุ่มตามบูสต์ใน BOX_MULTI_TARGETS
                   → ติ๊กบูสต์ → กด Play
  STATE 2 RUN    : ไม่กดอะไรเลย ปล่อยคุกกี้วิ่งเก็บกล่องเอง (ไม่กดกระโดด/ไม่กดนินจา relay)
  STATE 3 RESULT : เหมือนโหมดเดิม

โหมด "exp" (เก็บ EXP):
  STATE 1 PREP   : นำทางไปหน้าเตรียมตัว → ไม่รีโรล/ไม่ซื้ออะไร → ติ๊กเฉพาะ Star x2 (chk_star) → กด Play
  STATE 2 RUN    : วิ่งแบบเก็บกล่อง (ไม่กดกระโดด) — กดปุ่ม "Tap to activate Fast Start Boost!"
                   ตอนเริ่มวิ่ง + คอยกดนินจา relay จนจบด่าน
  STATE 3 RESULT : เหมือนโหมดเดิม
...
class CookieBot:
    def __init__(self, adb, log=print, on_coins=None, on_loop=None,
        ...
        #          / "exp" = เก็บ EXP (ไม่รีโรล ติ๊กเฉพาะ Star x2 แล้ววิ่งเหมือน coin)
        ...
        if self.mode not in ("coin", "box", "exp"):
    ...
    def _msleep(self, base):
        ...
        ขัดจังหวะได้เมื่อสั่งหยุด — ผู้ใช้กดหยุดแล้วบอทต้องหยุดแทบทันที ไม่ใช่รอหลับจบ"""
        self._pause(humanizer.menu_pause(base))

    def _pause(self, seconds):
        """หลับแบบขัดจังหวะได้ (เช็ค stop ทุก ~0.1s) — ใช้แทน time.sleep ทุกจุดในบอท"""
    ...
    def _required_templates(self):
        ...
        if self.mode == "box":
            # ต้องใช้ template ของ "ทุกช่อง" (รวมที่ไม่ได้เลือก) — ไว้ตรวจ+เอาติ๊กค้างออก
            req += [it["check_img"] for it in config.BOOST_ITEMS]
            if config.TAP_FAST_START_ACTIVATE:
                # ปิดใช้ Fast Start = ไม่ซื้อ+ไม่กด → template กลุ่มนี้ไม่บังคับ
                req += [config.IMG_FAST_START, config.IMG_FAST_START_BUY,
                        config.IMG_FAST_START_ACTIVATE]
            if config.BOX_MULTI_TARGETS:
                req += [config.IMG_MULTI_CHECK]   # ใช้เช็คติ๊กในหน้า Multi ตอนซื้อกล่องสุ่ม
        elif self.mode == "exp":
            # โหมด exp ติ๊กเฉพาะ Star x2 — template บูสต์อื่นไม่บังคับ
            # (ถ้ามีไฟล์ ensure_boosts_selected จะช่วยเอาติ๊กค้างของตัวอื่นออกให้ด้วย)
            req += [it["check_img"] for it in self._exp_boost_items()]
            if config.TAP_FAST_START_ACTIVATE:
                req += [config.IMG_FAST_START_ACTIVATE]
        else:
            req += [it["check_img"] for it in config.BOOST_ITEMS]
            req += [config.IMG_TARGET_ITEM, config.IMG_MULTI_CHECK]
        return req

    def _prep_boost_items(self):
        """บูสต์หน้าเตรียมตัวที่ผู้ใช้เลือกให้ติ๊ก (config.PREP_BOOST_TARGETS — ตั้งจากปุ่ม 🎁)
        กรองด้วยชื่อจาก BOOST_ITEMS — ชื่อที่สะกดไม่ตรงจะหายเงียบ จึงเตือนไว้ครั้งเดียว"""
        wanted = {str(n) for n in (config.PREP_BOOST_TARGETS or [])}
        items = [it for it in config.BOOST_ITEMS if str(it["name"]) in wanted]
        unknown = sorted(wanted - {str(it["name"]) for it in config.BOOST_ITEMS})
        if unknown and "prep_boost_names" not in self._tpl_warned:
            self._tpl_warned.add("prep_boost_names")
            self.log(f"[*][WARN] ไม่รู้จักบูสต์ใน PREP_BOOST_TARGETS: {', '.join(unknown)} "
                     "(ชื่อต้องตรงกับ BOOST_ITEMS — เช็ค settings.json)")
        return items

    def _exp_boost_items(self):
        """บูสต์ที่โหมด exp ติ๊ก — เฉพาะ Star x2 (chk_star.png) ตัวเดียว
        กรองจาก check_img (ไม่ใช่ชื่อ) กันเคสแก้ชื่อใน settings.json แล้วหาไม่เจอ"""
        return [it for it in config.BOOST_ITEMS
                if "chk_star" in str(it.get("check_img", ""))]
    ...
    def _nav_failed(self, msg):
        ...
        if self._stopping():
            return None   # ผู้ใช้สั่งหยุด — ไม่ใช่นำทางพัง อย่านับ streak/รอ 3 วิให้เสียเวลา
    ...
    # STATE 1 — REROLL
    # ============================================================
    def _multi_check_at(self, screen, cx, cy):
        """เช็คว่ามีเครื่องหมายถูกที่ตำแหน่ง (cx,cy) ในหน้า Multi ไหม — คืน (checked, score)"""
        r = config.MULTI_CHECK_ROI_R
        roi = (max(0, cx - r), max(0, cy - r), cx + r, cy + r)
        return vision.find_in_roi(screen, config.IMG_MULTI_CHECK, roi,
                                  config.MULTI_CHECK_THRESHOLD)

    def _multi_tick_targets(self, dlg, coords):
        """ติ๊กบูสต์ในหน้า "Pick desired Boosts!" ตามพิกัดที่ให้มา (โหมด coin)
        — เช็คก่อนว่าติ๊กอยู่แล้วหรือยัง (เกมจำติ๊กข้ามรอบ กดซ้ำจะ toggle ออก)"""
        for cx, cy in coords:
            checked, sc = self._multi_check_at(dlg, cx, cy)
            if checked:
    ...
    def _multi_ticked_names(self, screen, verbose=False):
        """สแกนหาเครื่องหมายถูก "ทั้งพื้นที่ตัวเลือก" ของหน้า Multi แล้วจับคู่แต่ละติ๊ก
        เข้ากับตัวเลือกที่ใกล้สุด — ทนพิกัดเยื้องกว่าเช็คทีละกรอบ (แถวห่าง ~49px
        ต่อให้พิกัดใน config เยื้องเป็นสิบ px การจับคู่ "ใกล้สุด" ก็ยังถูกตัว)
        คืน set ชื่อบูสต์ที่ติ๊กอยู่ | verbose=True log พิกัดติ๊กจริงที่เห็น (ไว้จูนพิกัด)"""
        hits = vision.find_all(screen, config.IMG_MULTI_CHECK,
                               config.MULTI_CHECK_THRESHOLD,
                               region=config.MULTI_DIALOG_REGION)
        ticked = set()
        for hx, hy, sc in hits:
            best, bd = None, None
            for opt in config.MULTI_BOOST_OPTIONS:
                cx, cy = tuple(opt["tap"])
                d = ((hx - cx) ** 2 + (hy - cy) ** 2) ** 0.5
                if bd is None or d < bd:
                    bd, best = d, str(opt["name"])
            if bd is not None and bd <= config.MULTI_MATCH_DIST:
                ticked.add(best)
                if verbose:
                    self.log(f"    เห็นติ๊กที่ ({hx},{hy}) = [{best}] "
                             f"(ห่างจุดตั้งค่า {bd:.0f}px, score={sc:.2f})")
            else:
                # ติ๊กจริงอยู่ไกลทุกจุดที่ตั้งไว้ = พิกัดใน config เยื้องแน่ ๆ — log พิกัดจริงไว้จูน
                self.log(f"    [WARN] เห็นติ๊กที่ ({hx},{hy}) แต่ไกลทุกตัวเลือก "
                         f"(ใกล้สุด [{best}] {bd:.0f}px) → พิกัด MULTI_BOOST_OPTIONS เยื้อง "
                         "ส่งบรรทัดนี้มาช่วยจูนได้เลย")
        return ticked

    def _multibuy_rolling(self, screen):
        """เกมกำลังสุ่มซื้อ (Multi-Buy rolling — ปุ่ม Stop สีส้มโชว์อยู่) ไหม
        คืน True/False เมื่อมี template ปุ่ม Stop | คืน None = ไม่มี template ตัดสินไม่ได้
        (ผู้เรียกต้องใช้วิธีสำรอง เช่น รอจอนิ่ง)"""
        if not vision.has_template(config.IMG_MULTI_STOP):
            if config.IMG_MULTI_STOP not in self._tpl_warned:
                self._tpl_warned.add(config.IMG_MULTI_STOP)
                self.log("[box] ยังไม่มีไฟล์ templates/multi_stop.png (ปุ่ม Stop สีส้มตอนสุ่มกล่อง) "
                         "→ ใช้วิธีรอจอนิ่งแทน — แคปปุ่ม Stop มาใส่เพิ่มจะแม่นขึ้น "
                         "(ผู้ใช้ .exe วางไฟล์ในโฟลเดอร์ templates ข้างโปรแกรมได้เลย)")
            return None
        return vision.find(screen, config.IMG_MULTI_STOP, 0.75).found

    def _save_multi_debug(self, screen):
        """เซฟภาพหน้า Multi ไว้ใน coin_logs ตอน sync ไม่สำเร็จ — ไว้เทียบจูนพิกัด/template"""
        if screen is None:
            return
        try:
            ...
            p = os.path.join(log_dir, f"multi_debug_{ts}.png")
            cv2.imwrite(p, screen)
            self.log(f"[box] เซฟภาพหน้า Multi ไว้ที่ {p} — ส่งภาพนี้มาช่วยจูนพิกัดได้")
        except Exception:
            pass

    def _multi_sync_boosts(self, wanted_names):
        """ปรับติ๊กในหน้า Multi ให้ตรงกับที่เลือก "ครบทั้ง 11 ตัวเลือก" (โหมด box):
          - ที่เลือกไว้แต่ยังไม่ติ๊ก → กดติ๊ก
          - ที่ "ไม่ได้เลือก" แต่ติ๊กค้างอยู่ (เกมจำจากรอบก่อน/มือกดเอง) → กดเอาออก
            ไม่เอาออก Multi-Buy จะหยุดที่บูสต์ผิดตัว = ได้ของที่ไม่ได้ตั้งใจ
        ตรวจด้วยการสแกนติ๊กทั้งหน้าแล้วจับคู่ตัวเลือกใกล้สุด (_multi_ticked_names)
        ทำสูงสุด 3 รอบ: แคปจอ → เทียบ → กดสลับตัวที่ไม่ตรง → แคปรอบใหม่ยืนยัน
        ไม่ตรงครบภายใน 3 รอบ → เซฟภาพ debug ไว้ให้จูน แล้วคืน False"""
        dlg = None
        for attempt in range(3):
            ...
            dlg = self._cap()
            if dlg is None:
                # ตาบอดห้ามกด — เดี๋ยว toggle ผิด (เหตุผลเดียวกับ guard หน้า Multi โหมด coin)
                self.log("    แคปหน้า Multi ไม่ได้ → รอแล้วลองใหม่")
                self._pause(1.0)
                continue
            ticked = self._multi_ticked_names(dlg, verbose=(attempt == 0))
            self.log(f"    สถานะติ๊ก: [{', '.join(sorted(ticked)) or 'ว่าง'}] | "
                     f"ต้องการ: [{', '.join(sorted(wanted_names))}]")
            if ticked == set(wanted_names):
                self.log("    ✔ ติ๊กตรงตามตั้งค่าครบทุกตัวแล้ว")
                return True
            for opt in config.MULTI_BOOST_OPTIONS:
                name = str(opt["name"])
                want, has = name in wanted_names, name in ticked
                if want == has:
                    continue
                self.log(f"    [{name}] {'ยังไม่ติ๊ก → กดเลือก' if want else 'ติ๊กค้าง/เกิน → กดเอาออก'}")
                self.adb.tap(*tuple(opt["tap"]), jitter=config.SAFE_TAP_JITTER)
                self._msleep(0.5)
            self._msleep(0.6)   # รอแอนิเมชันติ๊กนิ่งก่อนแคปรอบยืนยัน
        self.log("[box][WARN] ปรับติ๊ก 3 รอบแล้วยังไม่ตรง — พิกัดกดเยื้อง (กดไม่โดนปุ่ม) "
                 "หรือ templates/multi_check.png ไม่ตรงกับติ๊กจริง")
        self._save_multi_debug(dlg)
        return False
    ...
    def multibuy_until_target(self):
        ...
        if dlg is None:
            ...
            self._msleep(1.0)
            return False
        self._multi_tick_targets(dlg, config.MULTI_SELECT_TARGETS)
        if self._stopping():
            return False   # สั่งหยุดระหว่างติ๊ก — ห้ามกด Multi-Buy เสียเหรียญต่อ
    ...
    def ensure_boosts_selected(self, items=None):
        """
        ปรับติ๊กบูสต์หน้าเตรียมตัวให้ตรงกับที่ตั้งค่า "ครบทุกช่อง" ก่อนกด Play:
          - ตัวที่เลือกไว้แต่ยังไม่ติ๊ก → กดเปิดใช้
          - ตัวที่ "ไม่ได้เลือก" แต่ติ๊กค้างอยู่ → กดเอาออก
            (เกมจำติ๊กข้ามรอบ! ปล่อยไว้ = เสียของ/เหรียญกับบูสต์ที่ผู้ใช้ไม่ได้ตั้งใจใช้)
        items = รายการบูสต์ที่ต้องติ๊ก (default = ตาม PREP_BOOST_TARGETS; โหมด exp ส่ง Star x2)
        เช็คซ้ำสูงสุด 3 รอบ — รอบถัดไปแคปจอใหม่ยืนยันว่าสถานะเปลี่ยนจริง
        """
        if items is None:
            items = self._prep_boost_items()
        wanted = {str(it["name"]) for it in items}
        self.log(f"[*] ตรวจสอบ Boost (เลือกไว้ {len(wanted)}/{len(config.BOOST_ITEMS)}): "
                 + (", ".join(str(it["name"]) for it in items) if wanted else "ไม่ติ๊กเลย"))
        for attempt in range(3):
            if self._stopping():
                return
            screen = self._cap()
            ...
            for item in config.BOOST_ITEMS:
                name = str(item["name"])
                if not vision.has_template(item["check_img"]):
                    if item["check_img"] not in self._tpl_warned:
                        self._tpl_warned.add(item["check_img"])
                        self.log(f"    [{name}] ไม่มีไฟล์ {item['check_img']} → เช็ค/ปรับติ๊กไม่ได้ ข้าม")
                    continue
                checked, sc = vision.find_in_roi(screen, item["check_img"],
                                                 item["check_roi"], config.CHECK_THRESHOLD)
                want = name in wanted
                if checked == want:
                    if attempt == 0:   # log สถานะครั้งแรกพอ ไม่ให้รก
                        self.log(f"    [{name}] {'ติ๊กแล้ว' if want else 'ไม่ติ๊ก'} "
                                 f"ตรงตามตั้งค่า (score={sc:.2f})")
                    continue
                if want:
                    self.log(f"    [{name}] ยังไม่ติ๊ก (score={sc:.2f}) → กดเปิดใช้")
                else:
                    self.log(f"    [{name}] ติ๊กค้างทั้งที่ไม่ได้เลือก (score={sc:.2f}) → กดเอาออก")
                self.adb.tap(*item["tap"], jitter=config.SAFE_TAP_JITTER)
                self._msleep(0.7)
                all_ok = False
            if all_ok:
                self.log("[*] Boost ตรงตามตั้งค่าครบทุกช่องแล้ว")
                return
            self._msleep(0.5)   # รอแอนิเมชันติ๊กนิ่งก่อนแคปรอบยืนยัน
        self.log("[*][WARN] ปรับติ๊ก 3 รอบแล้วยังไม่ตรงครบ (ของหมด/พิกัดเยื้อง?) → ไปต่อ")
    ...
    def state_reroll(self):
        ...
        self._pause(random.uniform(*config.PLAY_PRESS_DELAY))   # คนขยับนิ้วไปหาปุ่ม
        if self._stopping():
            return None   # สั่งหยุดระหว่างหน่วง — ห้ามกด Play เริ่มเกมใหม่ทั้งที่ผู้ใช้สั่งหยุดแล้ว
    ...
    def ensure_fast_start(self):
        ...
        if slot.found:
            ...
            self._pause(random.uniform(*config.BOX_REACT_RANGE))   # เห็นก่อนแล้วค่อยขยับนิ้ว
            ...
            self._pause(random.uniform(*config.BOX_REACT_RANGE))
        ...
        self._pause(random.uniform(*config.BOX_REACT_RANGE))   # เห็นปุ่มก่อนแล้วค่อยกด
        if self._stopping():
            return False
    ...
    def multibuy_box_boosts(self):
        """
        โหมด box: ซื้อกล่อง Random Boost (1,200 เหรียญ/ครั้ง) ด้วย Multi-Buy
        ตามบูสต์ที่ผู้ใช้เลือกไว้ (config.BOX_MULTI_TARGETS — ตั้งจาก GUI ปุ่ม 🎁):
          1) เปิดกล่อง Random Boost → เปิดหน้า "Pick desired Boosts!"
          2) ติ๊กบูสต์ตามที่เลือก (อันที่ติ๊กค้างอยู่แล้วข้าม — กัน toggle ออก)
          3) กด Multi-Buy → เกมสุ่มซื้อซ้ำเองจนได้บูสต์ที่ติ๊ก แล้วหน้า Multi ปิดเอง
        สำเร็จ = หน้า Multi ปิด (ติ๊กที่เพิ่งกดหายไปพร้อม dialog + เห็นหน้าเตรียมตัว)
        คืน True เมื่อได้บูสต์ / ไม่ได้เลือกอะไรไว้ (ข้ามขั้นตอน)
        """
        wanted = {str(n) for n in (config.BOX_MULTI_TARGETS or [])}
        if not wanted:
            return True
        by_name = {str(o["name"]): tuple(o["tap"]) for o in config.MULTI_BOOST_OPTIONS}
        names = [n for n in by_name if n in wanted]
        coords = [by_name[n] for n in names]
        unknown = sorted(wanted - set(names))
        if unknown:
            self.log(f"[box][WARN] ไม่รู้จักบูสต์ใน BOX_MULTI_TARGETS: {', '.join(unknown)} "
                     "(ชื่อต้องตรงกับ MULTI_BOOST_OPTIONS — เช็ค settings.json)")
        if not coords:
            return True

        self.log(f"[box] ซื้อกล่อง Random Boost — Multi-Buy รอบูสต์: {', '.join(names)}")
        self.adb.tap(*config.BTN_BOX)
        self._msleep(0.8)
        self.adb.tap(*config.BTN_MULTI)
        self._msleep(1.0)

        # ปรับติ๊กให้ตรงกับที่เลือกครบทุกตัวเลือก (ติ๊กที่เลือก + เอาติ๊กค้างออก + ยืนยันซ้ำ)
        self._multi_sync_boosts(set(names))
        if self._stopping():
            return False   # สั่งหยุดระหว่างติ๊ก — ห้ามกด Multi-Buy เสียเหรียญต่อ

        self.log("[box] กด Multi-Buy → ให้เกมสุ่มซื้อเองจนได้บูสต์ที่เลือก")
        self.adb.tap(*config.BTN_MULTI_BUY)

        # รอจน "การสุ่มจบจริง" — กด Multi-Buy แล้ว dialog ปิด "ทันที" แต่เกมยังสุ่มซื้อ
        # ต่อบนหน้าเตรียมตัว (มีปุ่ม Stop สีส้มทับแถวปุ่ม Play จนกว่าจะได้บูสต์ที่ติ๊ก)
        # ห้ามถือว่า dialog ปิด = เสร็จ! บั๊กเดิมคือบอทรีบกด Play ระหว่างสุ่ม
        # → ไปโดนปุ่ม Stop → เกมตัดจบที่บูสต์ผิดตัว
        # วิธีเช็คสุ่มจบ: 1) ปุ่ม Stop หาย (ถ้ามี template) 2) ไม่มี → รอจอนิ่งติดกันหลายรอบ
        # (ตอนสุ่ม แบนเนอร์บูสต์เปลี่ยน/ปุ่ม Stop กะพริบตลอด จอไม่มีทางนิ่ง)
        start = time.time()
        last_sig = None
        stable = 0
        while time.time() - start < config.MULTIBUY_TIMEOUT:
            ...
            if screen is None:
                continue
            # ยังเห็นติ๊กในพื้นที่ตัวเลือก = dialog ยังเปิดอยู่ (ยังไม่เริ่มสุ่ม)
            if vision.find_all(screen, config.IMG_MULTI_CHECK,
                               config.MULTI_CHECK_THRESHOLD,
                               region=config.MULTI_DIALOG_REGION):
                last_sig, stable = None, 0
                continue
            rolling = self._multibuy_rolling(screen)
            if rolling is True:
                last_sig, stable = None, 0
                continue
            if rolling is None:
                # ไม่มี template ปุ่ม Stop → ใช้จอนิ่งติดกันหลายรอบเป็นสัญญาณสุ่มจบ
                sig = vision.frame_signature(screen)
                if (last_sig is not None
                        and float(abs(sig - last_sig).mean()) < config.MULTIBUY_STABLE_DIFF):
                    stable += 1
                else:
                    stable = 0
                last_sig = sig
                if stable < config.MULTIBUY_STABLE_COUNT:
                    continue
            if vision.find(screen, config.IMG_BOOST_SCREEN).found:
                self.log("[box] การสุ่มจบแล้ว — ได้บูสต์ที่เลือกจากกล่องสุ่ม")
                return True

        self.log("[box] หมดเวลา Multi-Buy (เหรียญไม่พอ/ติ๊กไม่ติด/จอไม่นิ่ง?) → ปิดหน้า Multi แล้วเล่นต่อ")
        self.adb.tap(*config.BTN_MULTI_CLOSE, jitter=config.SAFE_TAP_JITTER)
        self._msleep(1.0)
        return False

    def state_prepare_box(self):
        fs = "ซื้อ Fast Start" if config.TAP_FAST_START_ACTIVATE else "ไม่ใช้ Fast Start"
        self.log(f"\n===== [STATE 1] PREP — {fs} (โหมดวิ่งเก็บกล่อง ไม่รีโรล) =====")
        if not self.ensure_on_boost_screen():
            return self._nav_failed("นำทางยังไม่สำเร็จ → รอแล้ววนลองใหม่")
        ...
        if config.TAP_FAST_START_ACTIVATE:
            ...
            self.log("[box] ปิดใช้ Fast Start ไว้ → ไม่ซื้อ (เปิดได้ในแท็บ 🎁 ตั้งค่าบูสต์)")
        self._nav_fail_streak = 0

        # ซื้อกล่อง Random Boost ตามบูสต์ที่ผู้ใช้เลือก (ถ้าไม่ได้เลือกไว้ = ข้าม)
        if config.BOX_MULTI_TARGETS:
            self.multibuy_box_boosts()
            # กลับให้ถึงหน้าเตรียมตัวชัวร์ ๆ ก่อนติ๊กบูสต์ (เผื่อ toast/ป๊อปอัปหลังได้ของ)
            if not self.ensure_on_boost_screen():
                return self._nav_failed("หลังซื้อกล่องสุ่มกลับหน้าเตรียมตัวไม่ได้ → วนลองใหม่")

        self.ensure_boosts_selected()

        # กันเหนียว: ถ้าเกมยังสุ่มกล่องค้างอยู่ (ปุ่ม Stop ยังโชว์ — เคส timeout ข้างบน)
        # ห้ามกด Play เด็ดขาด ตำแหน่ง Play โดนปุ่ม Stop ทับ = ไปตัดจบการสุ่มผิดตัว
        for _ in range(12):
            if self._stopping():
                return None
            s = self._cap()
            if s is None or self._multibuy_rolling(s) is not True:
                break
            self.log("[box] เกมยังสุ่มกล่องอยู่ (ปุ่ม Stop ยังโชว์) → รอให้จบก่อนกด Play")
            self._pause(1.0)

        self.log("[OK] → กด Play เริ่มวิ่งเก็บกล่อง")
        self._pause(random.uniform(*config.PLAY_PRESS_DELAY))   # คนขยับนิ้วไปหาปุ่ม
        if self._stopping():
            return None   # สั่งหยุดระหว่างหน่วง — ห้ามกด Play เริ่มเกมใหม่
        self.adb.tap(*config.BTN_PLAY)
        self._msleep(config.DELAY_AFTER_PLAY)
        return State.RUN

    # ============================================================
    # โหมดเก็บ EXP (exp) — STATE 1: ไม่รีโรล ติ๊กเฉพาะ Star x2 แล้วกด Play
    # ============================================================
    def state_prepare_exp(self):
        self.log("\n===== [STATE 1] PREP — โหมดเก็บ EXP (ไม่รีโรล ติ๊กเฉพาะ Star x2) =====")
        if not self.ensure_on_boost_screen():
            return self._nav_failed("นำทางยังไม่สำเร็จ → รอแล้ววนลองใหม่")
        self._nav_fail_streak = 0

        items = self._exp_boost_items()
        if not items:
            self.log("[exp][WARN] ไม่พบบูสต์ chk_star ใน BOOST_ITEMS (เช็ค settings.json) "
                     "→ เริ่มเกมโดยไม่ติ๊กบูสต์")
        self.ensure_boosts_selected(items)
        self.log("[OK] → กด Play เริ่มวิ่งเก็บ EXP")
        self._pause(random.uniform(*config.PLAY_PRESS_DELAY))   # คนขยับนิ้วไปหาปุ่ม
        if self._stopping():
            return None   # สั่งหยุดระหว่างหน่วง — ห้ามกด Play เริ่มเกมใหม่
        self.adb.tap(*config.BTN_PLAY)
    ...
    # ============================================================
    # STATE 2 แบบไม่กระโดด — โหมด box (ปล่อยวิ่งเอง) และ exp (ปล่อยวิ่ง + กด relay)
    # ============================================================
    def state_run_box(self):
        """RUN แบบไม่กระโดด — ใช้ทั้งโหมด box และ exp
        ต่างกันแค่: exp กดนินจา relay จนจบด่าน / box ปล่อยผ่าน"""
        exp_mode = self.mode == "exp"
        fs = "เปิด Fast Start + " if config.TAP_FAST_START_ACTIVATE else "ไม่กด Fast Start, "
        if exp_mode:
            self.log(f"\n===== [STATE 2] RUN — {fs}ปล่อยวิ่งเอง (ไม่กระโดด) + คอยกดนินจา relay =====")
        else:
            self.log(f"\n===== [STATE 2] RUN — {fs}ปล่อยคุกกี้วิ่งเอง (ไม่กระโดด/ไม่กดนินจา) =====")
        start_time = time.time()
        last_sig = None
        last_change = time.time()
        ...
        while not self._stopping():
            ...
            # (ปิดได้จาก GUI — TAP_FAST_START_ACTIVATE=False = ปล่อยผ่านไม่กด)
            if config.TAP_FAST_START_ACTIVATE and t < config.FAST_START_ACTIVATE_WINDOW:
            ...
            # นินจา relay: exp = กดจนจบด่าน / box = "ไม่กด" (ไม่เปลี่ยนไม้ 2) log ไว้เฉย ๆ
            relay = vision.find(screen, config.IMG_RELAY, config.RELAY_THRESHOLD)
            if relay.found and exp_mode:
                # นินจาเป็น sprite วิ่งเคลื่อนที่ → tap ดิบรัว 3 ทีตรงจุดที่เจอ (เหมือนโหมด coin
                # — adb.tap() ที่ humanize เป็น swipe เกมไม่รับเป็นการแตะปุ่มนินจา)
                cx, cy = relay.center or config.BTN_RELAY
                for _ in range(3):
                    self.adb._input(["tap", cx, cy])
                    time.sleep(0.05)
                self.log(f"    [relay] เจอนินจาที่ ({cx},{cy}) score={relay.score:.3f} → กด")
                continue   # กดซ้ำรัว ๆ ตอนนินจายังโชว์ (ไม่รอ sleep ท้ายลูป)
            ...
            # โหมด exp ถี่กว่านั้นอีก (RELAY_POLL_INTERVAL) — นินจาโผล่แวบเดียว ต้องจับให้ทัน
            if exp_mode:
                time.sleep(config.RELAY_POLL_INTERVAL)
            else:
                time.sleep(0.2 if (config.TAP_FAST_START_ACTIVATE
                                   and t < config.FAST_START_ACTIVATE_WINDOW)
    ...
    def record_result_coins(self, screen, readable=True):
        ...
        โหมด exp อ่านตัวเลขแถว XP (XP_ROI) แทนแถว Coins — ค่าที่ลง CSV คือ EXP ของรอบนั้น
        readable=False = รู้อยู่แล้วว่าเฟรมนี้อ่านเลขไม่ได้ (ไม่ใช่หน้า Result / หา OK ไม่เจอ)
        → ลงแถวว่างกันรอบหายจาก log + เซฟภาพไว้ดู"""
        exp_mode = self.mode == "exp"
        unit = "EXP" if exp_mode else "เหรียญ"
        roi = config.XP_ROI if exp_mode else None
        coins = (self.coin_reader.read(screen, roi=roi)
                 if (readable and screen is not None) else None)
        ...
        try:
            ...
            if coins is not None:
                self.log(f"[coins] {unit}รอบนี้: {coins:,}  (รวม {self.coin_total:,})")
    ...
    def run(self, max_loops=0):
        ...
        mode_names = {"coin": "รีโรลเหรียญ (coin)",
                      "box": "วิ่งเก็บกล่อง (box — Fast Start)",
                      "exp": "เก็บ EXP (exp — Star x2, ไม่กระโดด, activate Fast Start + กด relay)"}
        self.log(f"[bot] โหมด: {mode_names.get(self.mode, self.mode)}")
            ...
            while not self._stopping():
                ...
                try:
                    if current == State.REROLL:
                        if box_mode:
                            current = self.state_prepare_box()
                        elif self.mode == "exp":
                            current = self.state_prepare_exp()
                        else:
                            current = self.state_reroll()
                    elif current == State.RUN:
                        # box และ exp วิ่งแบบไม่กระโดด (state_run_box) — exp กด relay เพิ่มในนั้น
                        current = self.state_run() if self.mode == "coin" else self.state_run_box()
